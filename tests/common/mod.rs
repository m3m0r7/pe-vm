//! Shared test utilities for pe-vm tests.

use pe_vm::{Architecture, Vm, VmConfig, VmError};

pub const TEST_BASE: u32 = 0x1000;

/// Test VM wrapper for instruction-level testing.
pub struct TestVm {
    pub vm: Vm,
}
...
impl TestVm {
    pub fn new(code: &[u8]) -> Self {
        ...
        vm.base = TEST_BASE;
        vm.memory = vec![0u8; mem_size];
        vm.memory[..code.len()].copy_from_slice(code);
        vm.regs.eip = TEST_BASE;
        vm.regs.esp = TEST_BASE + mem_size as u32 - 4;
        vm.stack_top = vm.regs.esp;
        vm.heap_start = 0x2000;
        vm.heap_end = 0x8000;
        vm.heap_cursor = vm.heap_start;
        vm.fs_base = TEST_BASE + 0x100;
        vm.gs_base = 0;
        Self { vm }
    }

    pub fn step(&mut self) -> Result<(), VmError> {
        pe_vm::windows::x86::X86Executor::new().step(&mut self.vm)
    }
    ...
    pub fn set_eip(&mut self, addr: u32) {
        self.vm.set_eip(addr);
    }

    pub fn reg32(&self, index: u8) -> u32 {
        self.vm.reg32(index)
    }

    pub fn set_reg32(&mut self, index: u8, value: u32) {
        self.vm.set_reg32(index, value);
    }

    pub fn reg16(&self, index: u8) -> u16 {
        self.vm.reg16(index)
    }

    pub fn set_reg16(&mut self, index: u8, value: u16) {
        self.vm.set_reg16(index, value);
    }
    ...
    pub fn set_flags(&mut self, zf: bool, sf: bool, of: bool, cf: bool) {
        self.vm.set_flags(zf, sf, of, cf);
    }

    pub fn read_u8(&self, addr: u32) -> Result<u8, VmError> {
        self.vm.read_u8(addr)
    }

    pub fn read_u32(&self, addr: u32) -> Result<u32, VmError> {
        self.vm.read_u32(addr)
    }
...
// PE fixtures for integration tests.
